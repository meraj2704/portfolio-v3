    ...
    <div className="flex flex-col min-h-[100dvh] bg-background text-foreground">
      ...
      <main className="flex-1 py-16 md:py-24">
        <div className="container px-4 md:px-6 max-w-6xl mx-auto">
          <Button variant="outline" className="mb-8 border-accent-primary text-accent-primary hover:bg-accent-primary hover:text-accent-primary-foreground transition-colors">
                    ...
                    {Array.from({ length: count }).map((_, index) => (
                      <button
                        ...
                        className={`h-2 w-2 rounded-full transition-colors ${
                          index + 1 === current ? 'bg-accent-primary' : 'bg-muted-foreground hover:bg-foreground'
                        }`}
              ...
              {project.images.map((image, index) => (
                <button
                  ...
                  className={`relative block w-full aspect-video rounded-md overflow-hidden cursor-pointer transition-all duration-200 ${
                    index + 1 === current ? 'ring-2 ring-accent-primary ring-offset-2 ring-offset-background' : 'hover:opacity-75'
                  }`}
              ...
              <div className="flex flex-col gap-4">
                ...
                {project.githubUrl && (
                  <Link href={project.githubUrl} target="_blank" rel="noopener noreferrer">
                    <Button variant="outline" className="w-full border-accent-primary text-accent-primary hover:bg-accent-primary hover:text-accent-primary-foreground transition-colors transform hover:scale-105">
                ...
                <>
                  ...
                  <ul className="list-disc list-inside space-y-2 text-muted-foreground">
                    {project.resources.map((resource, index) => (
                      <li key={index}>
                        <Link href={resource.url} target="_blank" rel="noopener noreferrer" className="hover:text-accent-primary underline-offset-4 hover:underline">
                          {resource.name}
                        </Link>
                      </li>
                    ))}
